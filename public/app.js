...
export function createDefaultGameState(bestScore = 0) {
    return {
        bestScore,
        chickenY: 0,
        distance: 0,
...
export function jump(state, config = defaultConfig) {
    if (state.status === "ready")
        return jump(startGame(state), config);
    if (state.status !== "running" || state.chickenY !== 0)
        return state;
    return { ...state, velocityY: config.jumpVelocity };
}
function shouldSpawnObstacle(state, config) {
    const lastObstacle = state.obstacles.at(-1);
    return !lastObstacle || lastObstacle.x < config.worldWidth - config.minObstacleGap;
}
function spawnObstacle(state, config) {
    if (!shouldSpawnObstacle(state, config))
        return state;
...
export function hasCollision(state, config = defaultConfig) {
    const chicken = {
        x: 160,
...
function drawGame(context, state, width, height) {
...
function initializeApp() {
    ...
    if (!maybeContext)
        throw new Error("Missing 2D canvas support");
